...
const StudioOverlay = ({ isOpen, onClose, product, requireLogin, initialMode = 'self', activeTemplateId = null, initial2DModelIdx = 0 }) => {
    ...
    const handleAnchorUpdate = useCallback((anchorData) => {
        ...
        if (activeItem) {
            ...
            toast.success("Frame Target Selected", { icon: '≡ƒÄ»', style: { borderRadius: '15px', background: '#0c0c2a', color: '#fff', fontSize: '10px', fontWeight: 'bold', textTransform: 'uppercase' } });
        }
    }, []);
                            ...
                            <div className="floating-card flex-1 p-8 flex flex-col gap-6 overflow-y-auto no-scrollbar">
                                ...
                                <div className="p-6 bg-slate-50/80 rounded-[32px] space-y-4">
                                    <div className="flex justify-between items-center text-[11px] font-bold">
                                        ...
                                        <span className="text-slate-900 bg-white px-3 py-1.5 rounded-lg border border-slate-100 shadow-sm text-sm">Γé╣ {(product.discountPrice || product.basePrice || 0).toLocaleString()}</span>
                                    </div>
                                                ...
                                                <tbody className="divide-y divide-slate-50">
                                                    {product.bulkRules.sort((a, b) => a.minQty - b.minQty).map((rule, idx) => (
                                                        <tr key={idx} className="text-slate-500">
                                                            ...
                                                            <td className="px-3 py-2 text-right">Γé╣{rule.pricePerUnit}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                                ...
                                <div className="mt-auto space-y-4">
                                    <div className="flex justify-between items-end pb-2">
                                        ...
                                        <span className="text-2xl font-black text-[#0c0c2a]">Γé╣ {((product.discountPrice || product.basePrice || 0) * variations.length).toLocaleString()}</span>
                                    </div>
                        ...
                        <div className="mt-8 w-full bg-slate-900 rounded-3xl p-6 flex items-center justify-between shadow-2xl animate-in slide-in-from-bottom-4">
                            <div className="flex items-center gap-6">
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-xl font-black text-white">Γé╣ {(product.discountPrice || product.basePrice || 0).toLocaleString()}</span>
                                </div>

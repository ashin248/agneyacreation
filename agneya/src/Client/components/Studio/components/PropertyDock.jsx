        ...
        <div className="xl:hidden fixed bottom-0 left-0 right-0 z-[600] pointer-events-none flex flex-col justify-end">
            ...
            <div className={`bg-white/95 backdrop-blur-xl rounded-t-[32px] shadow-[0_-20px_60px_rgba(0,0,0,0.1)] transition-transform duration-500 pointer-events-auto flex flex-col border-t border-slate-100 ${isMobileUiMinimized || !activeObject ? 'translate-y-full' : 'translate-y-0'}`} style={{ maxHeight: '60vh', paddingBottom: '90px' }}>
                ...
                <div className="flex justify-between items-center px-6 pt-8 pb-4 border-b border-slate-100 shrink-0">
                    ...
                    <div className="flex items-center gap-3">
                        {activeObject && (
                            <button onClick={() => { fabricRef.current.discardActiveObject(); fabricRef.current.renderAll(); setActiveObject(null); setIsMobileUiMinimized(true); }} className="px-4 py-2 bg-orange-50 text-orange-600 rounded-full text-[9px] font-black uppercase tracking-tight active:scale-95 transition-all">Deselect</button>
                        )}
                        <button onClick={() => setIsMobileUiMinimized(true)} className="w-8 h-8 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-900 shadow-sm">
                            <FiArrowDown size={14} />
                        </button>
                    </div>
                </div>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="grid grid-cols-2 gap-x-8 gap-y-6">
                                <div className="space-y-3">
                                    <div className="flex justify-between text-[10px] font-black text-slate-900 uppercase tracking-tighter"><span>Size</span><span>{Math.round(activeObject.scaleX * 100)}%</span></div>
                                    ...
                                    }} onMouseUp={() => updateTexture(true)} className="w-full accent-orange-600" />
                                </div>
                                <div className="space-y-3">
                                    <div className="flex justify-between text-[10px] font-black text-slate-900 uppercase tracking-tighter"><span>Rotate</span><span>{Math.round(activeObject.angle)}°</span></div>
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between text-[10px] font-black text-slate-900 uppercase tracking-tighter"><span>Pos X</span><span>{Math.round(activeObject.left)}</span></div>
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between text-[10px] font-black text-slate-900 uppercase tracking-tighter"><span>Pos Y</span><span>{Math.round(activeObject.top)}</span></div>
                                ...
                                <div className="space-y-3 col-span-2">
                                    <div className="flex justify-between text-[10px] font-black text-slate-900 uppercase tracking-tighter"><span>Transparency</span><span>{Math.round(activeObject.opacity * 100)}%</span></div>
                            ...
                            {(activeObject.type === 'i-text' || activeObject.type === 'text') && (
                                <div className="space-y-4">
                                    ...
                                    }} className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-[14px] font-bold text-slate-900 outline-none" placeholder="Enter text..."></textarea>
                                </div>
                            )}
                                ...
                                <div className="grid grid-cols-5 gap-3">
                                    {['#F7941D', '#3b82f6', '#ec4899', '#fbbf24', '#ffffff', '#ef4444', '#10b981', '#6366f1', '#f97316', '#000000'].map((color, i) => (
                                        <button key={i} onClick={() => {
                                            ...
                                            setBrushColor(color);
                                        }} className={`aspect-square rounded-full border-2 transition-all ${brushColor === color ? 'border-orange-500 scale-110 shadow-md' : 'border-slate-100'}`} style={{ backgroundColor: color }}></button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    {(activeObject.type === 'image' || activeObject.type === 'FabricImage' || activeObject.uid?.startsWith('upload_') || activeObject.uid?.startsWith('up_')) && (
                                        ...
                                        }} className="h-12 bg-orange-50 border border-orange-100 text-orange-600 text-[9px] font-black uppercase rounded-xl flex items-center justify-center gap-2">
                                    ...
                                    {activeStudioTab === '2D_STUDIO' && (activeObject.type === 'image' || activeObject.type === 'FabricImage' || activeObject.uid?.startsWith('upload_')) && (
                                        ...
                                        }} className={`h-12 border text-[9px] font-black uppercase rounded-xl flex items-center justify-center gap-2 ${activeObject.bringToFront ? 'bg-orange-50 border-orange-100 text-orange-600' : 'bg-slate-50 border-slate-100 text-slate-900'}`}>
                                            <FiLayers size={14} /> Layer
                                        </button>
                                    )}
                                    <button onClick={() => { fabricRef.current.centerObject(fabricRef.current.getActiveObject()); fabricRef.current.renderAll(); updateTexture(); }} className="h-12 bg-slate-50 border border-slate-100 text-slate-900 text-[9px] font-black uppercase rounded-xl flex items-center justify-center gap-2"><FiMove size={14} /> Center</button>
            ...
            {activeObject && isMobileUiMinimized && (
                <div className="absolute bottom-24 left-1/2 -translate-x-1/2 pointer-events-auto z-50">
                    <button 
                        ...
                        className="px-6 py-3 text-white rounded-full shadow-[0_10px_20px_rgba(247,148,29,0.3)] flex items-center gap-2 text-[10px] font-black uppercase tracking-widest animate-bounce"
            ...
            <div className="absolute bottom-0 left-0 right-0 h-20 bg-white border-t border-slate-100 flex items-center justify-around px-2 pb-safe pointer-events-auto shadow-[0_-10px_30px_rgba(0,0,0,0.05)] z-40">
                {[
                    ...
                    <button key={tab.id} onClick={() => { setActiveTab(tab.id); if (tab.id !== 'draw') setIsDrawing(false); }} className={`flex-1 h-full flex flex-col items-center justify-center gap-1 transition-all ${activeTab === tab.id ? 'text-orange-600' : 'text-slate-400 hover:text-orange-500'}`}>
                        <div className={`p-2 rounded-xl transition-all ${activeTab === tab.id ? 'bg-orange-50 scale-110' : ''}`}>
                            {tab.icon}
                        </div>

...
import { FiSave, FiCheckCircle, FiXCircle, FiClock, FiSettings, FiImage, FiDownload, FiAlertTriangle, FiRotateCw, FiMaximize2, FiSearch, FiPackage } from 'react-icons/fi';
...
function ModelInspector({ design, onLoad, onError }) {
    ...
    // Reconstruction Engine (Internal to ModelInspector to keep scope clean)
    useEffect(() => {
        if (!design || !design.frontCanvasData) {
        ...
        const reconstruct = async () => {
            try {
                const canvasData = typeof design.frontCanvasData === 'string' 
                    ? JSON.parse(design.frontCanvasData) 
                    : design.frontCanvasData;
                ...
                const snapshots = canvas.getObjects().filter(o => !o.excludeFromExport).map(obj => {
                    return {
                        ...
                        offsetX: ((obj.getCenterPoint().x) - (design.canvasWidth || 500) / 2) / ((design.canvasWidth || 500) / 2),
                        offsetY: ((obj.getCenterPoint().y) - (design.canvasHeight || 600) / 2) / ((design.canvasHeight || 600) / 2),
                        rotation: obj.angle || 0,
                        scaleX: obj.scaleX || 1,
                        scaleY: obj.scaleY || 1,
                        width: obj.width,
                        height: obj.height,
                        canvasWidth: design.canvasWidth || 500,
                        canvasHeight: design.canvasHeight || 600
                    };
                });
                
                setReconstructedObjects(snapshots);
                setReconstructedAnchors(design.frontAnchors || {});
            } catch (err) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[780px]">
                ...
                <div className="lg:col-span-3 bg-white/80 backdrop-blur-xl rounded-[32px] border border-white/40 shadow-xl overflow-hidden flex flex-col border-gray-100">
                    <div className="flex justify-between items-center mb-6 p-6 pb-0">
                        <h2 className="text-lg font-bold uppercase tracking-tight">Studio Hub</h2>
                    ...
                    <div className="flex-1 overflow-y-auto p-1 space-y-3 custom-scrollbar">
                        {loading ? (
                            ...
                            <button
                                ...
                                className={`w-full text-left p-4 rounded-[24px] border transition-all ${selectedDesign?._id === design._id ? 'ring-2 ring-slate-900 shadow-md' : 'border-transparent hover:border-gray-200'} ${
                                    design.status === 'Approved' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' :
                                    design.status === 'Rejected' ? 'bg-red-50 text-red-600 border-red-200' :
                                ...
                                <div className="flex gap-4 items-center relative">
                                    ...
                                    <div className="flex-1 overflow-hidden">
                                        <div className="flex items-center justify-between mb-1">
                                            <span className="text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-white/10">
                    ...
                    {!selectedDesign ? (
                        ...
                        <div className="flex flex-col lg:flex-row h-full">
                            <div className="lg:w-[65%] border-r border-gray-100 relative bg-[#f8f9fb]">
                                <div className="h-full flex flex-col overflow-y-auto no-scrollbar custom-scrollbar">
                                    {/* 1. PROJECT BRIEF */}
                                    <div className="p-8 sm:p-12 border-b border-gray-100 bg-white">
                                        <div className="flex items-center gap-3 mb-6">
                                            <div className="w-1.5 h-1.5 rounded-full bg-indigo-500"></div>
                                            <span className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.4em]">Project Brief</span>
                                        </div>
                                        <p className="text-2xl font-black text-slate-900 leading-tight uppercase tracking-tighter italic">
                                            {selectedDesign.description || "NO WRITTEN INSTRUCTIONS PROVIDED BY CLIENT."}
                                        </p>
                                    </div>

                                    {/* 2. VISUALIZER */}
                                    <div className="relative min-h-[500px] flex-shrink-0 bg-[#f1f3f6]">
                                        {reconstructing && (
                                            <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/50 backdrop-blur-sm">
                                                ...
                                                <p className="mt-4 text-[10px] font-black uppercase tracking-widest text-blue-600">Syncing Matrix...</p>
                                            </div>
                                        )}
                                        ...
                                        {(selectedDesign.frontCanvasData || selectedDesign.backCanvasData) ? (
                                            <>
                                                <div className="absolute top-6 left-6 z-20 flex items-center gap-3">
                                                    <div className="bg-white/80 backdrop-blur px-4 py-2 rounded-full shadow-sm border border-gray-100 flex items-center gap-2">
                                                        ...
                                                        <span className="text-[9px] font-black tracking-widest text-gray-800 uppercase">Interactive 3D Preview</span>
                                                    </div>
                                                </div>
                                                ...
                                                >
                                                    <Stage environment="city" intensity={0.5} contactShadow={{ opacity: 0.2 }}>
                                                        <ModelInspector 
                                                            ...
                                                            onLoad={() => setReconstructing(false)} 
                                                            onError={() => setReconstructing(false)} 
                                                        />
                                                    </Stage>
                                            ...
                                            <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-4">
                                                ...
                                                <div className="text-center">
                                                    <p className="text-[10px] font-black uppercase tracking-[0.3em]">Manual Brief Mode</p>
                                                    <p className="text-[8px] font-bold uppercase tracking-widest opacity-60">No 3D canvas data identified</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="p-8 sm:p-12 bg-white">
                                        <div className="flex items-center justify-between mb-8">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <h3 className="text-xs font-black text-slate-900 uppercase tracking-[0.2em]">Reference Resource Library</h3>
                                            </div>
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                                           {selectedDesign.printAssets?.map((asset, idx) => (
                                              <div key={idx} className="group relative aspect-square bg-slate-50 rounded-[32px] border-4 border-white shadow-xl overflow-hidden hover:scale-105 transition-all">
                                                 <img loading="lazy" src={asset} className="w-full h-full object-cover" alt="Reference" />
                                                 <div className="absolute inset-0 bg-slate-900/80 opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-center p-4 text-center">
                                                    ...
                                                    <span className="text-[9px] font-black text-white uppercase tracking-widest">Open High-Res</span>
                                                 </div>
                                              </div>
                                           ))}
                                           {(!selectedDesign.printAssets || selectedDesign.printAssets.length === 0) && (
                                              <div className="col-span-full py-20 bg-slate-50 border border-dashed border-slate-200 rounded-[40px] flex flex-col items-center justify-center gap-4 text-slate-300">
                                                 <FiImage size={32} />
                            ...
                            <div className="lg:w-[35%] flex flex-col">
                                <div className="p-8 border-b border-gray-50">
                                    <h2 className="text-xl font-black uppercase tracking-tight">Deployment Data</h2>
                                    <p className="text-[10px] uppercase tracking-widest text-gray-400 font-bold mt-1 tracking-[0.2em]">Sync Status: Secured</p>
                                </div>
                                
                                <div className="flex-1 p-8 space-y-6 overflow-y-auto custom-scrollbar">
                                    <div className="bg-slate-900 text-white p-6 rounded-[2rem] space-y-4 shadow-xl">
                                        <div className="flex justify-between items-center"><span className="text-[9px] font-bold uppercase text-slate-400 tracking-widest">Client</span><span className="text-[11px] font-black truncate max-w-[140px]">{selectedDesign.email}</span></div>
                                        <div className="flex justify-between items-center"><span className="text-[9px] font-bold uppercase text-slate-400 tracking-widest">Type</span><span className="text-[11px] font-black">{selectedDesign.productType}</span></div>
                                        <div className="flex justify-between items-center"><span className="text-[9px] font-bold uppercase text-slate-400 tracking-widest">Quantity</span><span className="text-[11px] font-black">{selectedDesign.quantity} Units</span></div>
                                    </div>

                                    <div>
                                        <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest block mb-3 ml-2">Internal Notes</label>
                                        <textarea
                                            value={adminNotes}
                                            onChange={(e) => setAdminNotes(e.target.value)}
                                            placeholder="PRODUCTION NOTES..."
                                            className="w-full h-32 bg-gray-50 border-none rounded-2xl p-4 text-xs font-bold focus:ring-2 focus:ring-indigo-100 transition-all resize-none"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="p-8 bg-white border-t border-gray-100 space-y-3 relative z-10">
                                    ...
                                    >
                                        <FiCheckCircle /> {selectedDesign.status === 'Approved' ? 'Confirmed' : 'Authorize Design'}
                                    </button>
                                    ...
                                    >
                                         {selectedDesign.status === 'Rejected' ? 'Rejected' : 'Reject Assignment'}
                                    </button>
                                </div>
                            </div>
                        </div>

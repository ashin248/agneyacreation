...
const Navbar = () => {
  ...
  return (
    <div className="flex flex-col w-full sticky top-0 z-[100]">
      <nav className="bg-white/80 backdrop-blur-3xl border-b border-slate-100 h-24 flex items-center">
        <div className="max-w-7xl mx-auto px-6 w-full flex justify-between items-center">
            ...
            <div className="flex items-center">
              <Link to="/" className="group flex items-center gap-4">
                <div className="relative">
                    <img src={logoImg} alt="Agneya" className="h-12 w-auto object-contain transition-all duration-700 group-hover:scale-105" />
            ...
            <div className="hidden md:flex items-center bg-slate-50 p-1.5 rounded-2xl border border-slate-100">
              {[
                ...
                <NavLink 
                  key={item.name}
                  to={item.path} 
                  className={({ isActive }) => `flex items-center gap-3 px-8 py-3.5 rounded-xl transition-all duration-500 ${
                    item.highlight 
                ...
                >
                  <item.icon size={16} className={item.highlight ? 'animate-pulse' : ''} />
            ...
            <div className="flex items-center gap-5">
              {currentUser || userData ? (
                <Link to="/dashboard" className="hidden lg:flex items-center gap-4 pl-2 pr-6 py-2 bg-slate-50 rounded-2xl border border-slate-100 group hover:bg-slate-950 transition-all duration-500 shadow-sm">
                  <div className="w-12 h-12 rounded-xl bg-white shadow-sm flex items-center justify-center text-slate-950 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                    <User size={18} />
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest group-hover:text-white/40">Hello,</span>
                    <span className="text-[11px] font-black text-slate-900 uppercase tracking-tighter group-hover:text-white truncate max-w-[90px]">
        ...
        <div className={`md:hidden absolute top-24 left-0 w-full bg-white/95 backdrop-blur-3xl border-b border-slate-100 shadow-2xl transition-all duration-500 ${
            ...
            <div className="p-8 space-y-6">
              {[
                ...
                <Link 
                    ...
                    className="flex items-center gap-5 p-6 bg-slate-50 rounded-[32px] border border-slate-100 group active:bg-slate-950 active:text-white transition-all"
                >
                    <div className="w-16 h-16 rounded-[20px] bg-white flex items-center justify-center text-slate-400 group-active:bg-white/10 group-active:text-white transition-all shadow-sm">
                        <item.icon size={26} />
                    </div>
                    <div>
                        <h4 className="text-[13px] font-black uppercase tracking-[0.2em] text-slate-900 group-active:text-white">{item.name}</h4>
                        <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-1 group-active:text-white/40">{item.desc}</p>
                    </div>
                </Link>
              ))}
